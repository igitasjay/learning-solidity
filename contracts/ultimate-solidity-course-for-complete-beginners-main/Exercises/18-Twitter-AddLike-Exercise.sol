...
contract Twitter {
    ...
    // function texting(id) external {}

    function likeTweet(address author, uint256 id) external {
        require(tweets[author][id].id == id, "tweet does not exist after all.");
        tweets[author][id].likes++;
    }

    function unlikeTweet(address author, uint256 id) external {
        require(tweets[author][id].id == id, "tweet does not exist after all.");
        require(tweets[author][id].likes > 0, "you can't unlike a tweet that has 0 likes");
        tweets[author][id].likes--;
    }
